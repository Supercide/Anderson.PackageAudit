...
namespace Anderson.PackageAudit.Vulnerabilities.Pipes
{
    ...
    public class GetProjectVulnerabilitiesPipe : PipelineDefinition<ProjectVulnerabilitiesRequest>
    {
        ...
        public GetProjectVulnerabilitiesPipe(IMongoCollection<Vulnerability> vulnerabilitiesCollection, IMongoCollection<Tenant> tenantsCollection)
        {
            _vulnerabilitiesCollection = vulnerabilitiesCollection;
            _tenantsCollection = tenantsCollection;
        }

        public override Task HandleAsync(ProjectVulnerabilitiesRequest request, Context context, CancellationToken token = default(CancellationToken))
        {
            if (IsAuthorizedForTenant(request, context))
            {
                var vulnerabilities = _vulnerabilitiesCollection.Find(x => x.Project == request.Project && x.Tenant == request.Tenant)
                    .ToList()
        ...
        private bool IsAuthorizedForTenant(ProjectVulnerabilitiesRequest request, Context context)
        {
